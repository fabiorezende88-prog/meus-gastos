const K="meus_gastos_v3",BK="meus_gastos_orcamento";let E=JSON.parse(localStorage.getItem(K)||"[]"),B=JSON.parse(localStorage.getItem(BK)||"{}"),M=new Date().toISOString().slice(0,7),T="variable",C="🍔 Alimentação",editingId=null;
...
function render(){let L=E.filter(x=>x.date.slice(0,7)==M).sort((a,b)=>b.created-a.created),tot=L.reduce((s,x)=>s+x.value,0),fix=L.filter(x=>x.type=="fixed").reduce((s,x)=>s+x.value,0);$("monthBtn").textContent=ml(M);$("total").textContent=money(tot);$("fixed").textContent=money(fix);$("variable").textContent=money(tot-fix);
$("list").innerHTML=L.length?L.map(x=>`<div class="expense"><div class="icon">${iconFor(x.category)}</div><div class="main"><b>${esc(x.description||x.category.slice(2))}</b><span>${x.type=="fixed"?"Fixa":"Variável"} · ${br(x.date)}</span></div><div class="amount">${money(x.value)}<button class="edit" onclick="edit('${x.id}')">✏️ Editar</button><button class="delete" onclick="del('${x.id}')">Excluir</button></div></div>`).join(""):`<div class="empty">Nenhuma despesa neste mês.<br>Toque em <b>+ GASTO</b> para começar.</div>`;
...
function selMonth(m){M=m;$("months").classList.add("hidden");render()}render();